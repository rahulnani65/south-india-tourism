...
class Command(BaseCommand):
    help = 'Update image URLs for places'
            ...
            for place_name, image_url in place_images.items():
                try:
                    ...
                    if place:
                        ...
                        updated_count += 1
                        self.stdout.write(
                            self.style.SUCCESS(f'✓ Updated {place.name} with image URL')
                        )
                    else:
                        not_found_count += 1
                    ...
                    self.stdout.write(
                        self.style.ERROR(f'✗ Error updating {place_name}: {str(e)}')
                    )

        # Summary
        self.stdout.write('\n' + '='*50)
        self.stdout.write(self.style.SUCCESS(f'✓ Successfully updated {updated_count} places'))
        
        if not_found_count > 0:
            self.stdout.write(self.style.WARNING(f'⚠ {not_found_count} places not found:'))
            for place in not_found_places:
                self.stdout.write(f'  - {place}')
        
        self.stdout.write('='*50)
        
        # Show all places with their current image URLs
        self.stdout.write('\nCurrent places with image URLs:')
        places_with_images = Place.objects.exclude(image_url__isnull=True).exclude(image_url='')
        for place in places_with_images:
            self.stdout.write(f'  ✓ {place.name} ({place.state.name}): {place.image_url[:50]}...') 